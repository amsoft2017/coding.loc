...
class onLotteryContentAfterAdd extends cmsAction
{

    public function run($data)
    {

        $lottos = $this->model->filter('i.start_date >= NOW()')->filterEqual('is_content', 1)->filterEqual('winner', null)->getLottos();
        ...
        if ($lottos) {
            ...
            foreach ($lottos as $lotto) {
                ...
                if (!$players) {
                    ...
                    $user_coms = $this->model->filterEqual('id_lotto', $lotto['id'])->filterEqual('id_users', $user['id'])->getContents();
                    ...
                    if (!$user_coms) {
                        ...
                        $com_data['round'] = 1;
                        $com_data_id = $this->model->addContent($com_data);

                        if ($com_data['count'] == $lotto['count_content']) {
                            ...
                            $this->model->updateContent($com_data_id['id'], $com_data);
                        ...
                        foreach ($user_coms as $user_com) {
                            ...
                            if ($user_com['count'] < $lotto['count_content']) {

                                $user_com['count']++;

                                $this->model->updateContent($user_com['id'], $user_com);
                            }
                    ...
                    foreach ($players as $player) {
                        ...
                        if (!$user_coms) {
                            ...
                            foreach ($user_coms as $user_com) {
                                ...
                                $count = $lotto['count_content'] * $user_com['round'];
